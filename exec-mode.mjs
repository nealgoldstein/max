      ...
      (function(sid,d,btn){calBtn.onclick=function(e){
        ...
        setTimeout(function(){
          document.addEventListener("click",function dismiss(ev){
            if(!picker.contains(/** @type {any} */ (ev.target))){picker.parentNode&&picker.parentNode.removeChild(picker);document.removeEventListener("click",dismiss);}
          });
        },10);
      };})(s.id,dest,calBtn);
   ...
   in the gate. Removed when the import-rewiring phase lands real imports. */
{
  ...
  __expg._exFullMap = _exFullMap;
  __expg._exMapInstance = _exMapInstance;
  __expg._exSheetOpen = _exSheetOpen;
  __expg.calcRouteSummary = calcRouteSummary;
  __expg.drawExecMode = drawExecMode;
  __expg.getAllSights = getAllSights;
  __expg.getTodayIds = getTodayIds;
  __expg.isToday = isToday;
  __expg.mkExSight = mkExSight;
  __expg.saveTodayIds = saveTodayIds;
  __expg.setDestMode = setDestMode;
  __expg.shouldAutoExec = shouldAutoExec;
  __expg.switchTab = switchTab;
}
