   ...
   in the gate. Removed when the import-rewiring phase lands real imports. */
{
  ...
  __expg.IDB_NAME = IDB_NAME;
  __expg.IDB_STORE = IDB_STORE;
  __expg.KEY_API_KEY = KEY_API_KEY;
  __expg.KEY_DRAFT_PREFIX = KEY_DRAFT_PREFIX;
  __expg.KEY_GEOCODE = KEY_GEOCODE;
  __expg.KEY_HIDE_INTRO = KEY_HIDE_INTRO;
  __expg.KEY_LLM_CACHE = KEY_LLM_CACHE;
  __expg.KEY_MAP_STYLE = KEY_MAP_STYLE;
  __expg.KEY_PREFS = KEY_PREFS;
  __expg.KEY_TRIPS_INDEX = KEY_TRIPS_INDEX;
  __expg.KEY_TRIP_PREFIX = KEY_TRIP_PREFIX;
  __expg.KEY_WIKI_MIGRATED = KEY_WIKI_MIGRATED;
  __expg.KEY_WIKI_PREFIX = KEY_WIKI_PREFIX;
  __expg.MaxDB = MaxDB;
  __expg.WIKI_TTL_MS = WIKI_TTL_MS;
  __expg._evictLowPriorityKeys = _evictLowPriorityKeys;
  __expg._hydrationPromise = _hydrationPromise;
  __expg._prefsCache = _prefsCache;
  __expg._prefsLoad = _prefsLoad;
  __expg._prefsMigrated = _prefsMigrated;
  __expg._prefsPersist = _prefsPersist;
  __expg._tripIdbMirror = _tripIdbMirror;
  __expg._tripIdbMirrorRaw = _tripIdbMirrorRaw;
  __expg._tripIdbReady = _tripIdbReady;
  __expg._writeTripToIdb = _writeTripToIdb;
  __expg.canPersist = canPersist;
  __expg.cityDataClear = cityDataClear;
  __expg.cityDataDelete = cityDataDelete;
  __expg.cityDataGet = cityDataGet;
  __expg.cityDataHas = cityDataHas;
  __expg.cityDataMap = cityDataMap;
  __expg.cityDataMem = cityDataMem;
  __expg.cityDataSet = cityDataSet;
  __expg.cleanupOrphaned = cleanupOrphaned;
  __expg.draftDelete = draftDelete;
  __expg.draftRead = draftRead;
  __expg.draftWrite = draftWrite;
  __expg.geocodeAll = geocodeAll;
  __expg.geocodeFlush = geocodeFlush;
  __expg.geocodeGet = geocodeGet;
  __expg.geocodeLoad = geocodeLoad;
  __expg.geocodeMem = geocodeMem;
  __expg.geocodeReplace = geocodeReplace;
  __expg.geocodeSet = geocodeSet;
  __expg.hydrateTripIdbMirror = hydrateTripIdbMirror;
  __expg.idbDelete = idbDelete;
  __expg.idbGet = idbGet;
  __expg.idbOpen = idbOpen;
  __expg.idbSet = idbSet;
  __expg.inIframe = inIframe;
  __expg.indexEntry = indexEntry;
  __expg.indexLoad = indexLoad;
  __expg.indexRemove = indexRemove;
  __expg.indexSave = indexSave;
  __expg.indexUpsert = indexUpsert;
  __expg.llmCacheAll = llmCacheAll;
  __expg.llmCacheClear = llmCacheClear;
  __expg.llmCacheGet = llmCacheGet;
  __expg.llmCacheMem = llmCacheMem;
  __expg.llmCacheReplace = llmCacheReplace;
  __expg.llmCacheSet = llmCacheSet;
  __expg.llmReady = llmReady;
  __expg.prefsClear = prefsClear;
  __expg.prefsGet = prefsGet;
  __expg.prefsReplace = prefsReplace;
  __expg.prefsSet = prefsSet;
  __expg.settingGet = settingGet;
  __expg.settingRemove = settingRemove;
  __expg.settingSet = settingSet;
  __expg.storageStats = storageStats;
  __expg.tripDelete = tripDelete;
  __expg.tripHydrated = tripHydrated;
  __expg.tripListIds = tripListIds;
  __expg.tripRead = tripRead;
  __expg.tripReadRaw = tripReadRaw;
  __expg.tripWrite = tripWrite;
  __expg.tripWriteRaw = tripWriteRaw;
  __expg.wikiCacheGet = wikiCacheGet;
  __expg.wikiCacheMigrate = wikiCacheMigrate;
  __expg.wikiCacheSet = wikiCacheSet;
}
