...
test.describe('Build harness — canned-LLM end-to-end', () => {
  ...
  // PD.401Q: keep-enforcement uses the SAME bidirectional identity as
  // coverage. A one-word listed SIGHT that the LLM expanded ("Þingvellir"
  // → "Þingvellir National Park") must be committed (keep=true), not left
  // at the LLM's keep=false. And a listed STAY ("Reykjavik") must NEVER
  // reverse-match its own sub-place sight ("Reykjavik Old Harbour") —
  // that would silently auto-check a Max suggestion ("Max never checks").
  test('PD.401Q: listed sight\'s expanded twin is committed; a stay\'s sub-place is not', async ({ page }) => {
    await bootClean(page);
    await runPipeline(page);
    const r = await page.evaluate(() => {
      window._tb._userListedNames = {};
      window._tb._userListedDisplay = {};
      const tk = window._pmKey('Þingvellir');
      window._tb._userListedNames[tk] = 'see';
      window._tb._userListedDisplay[tk] = 'Þingvellir';
      const rk = window._pmKey('Reykjavik');
      window._tb._userListedNames[rk] = 'stay';
      window._tb._userListedDisplay[rk] = 'Reykjavik';

      // The expanded sight (Max named it; keep=false) and a true sub-place.
      window._tb.placeActivities.push({
        id: 'q-expand', type: 'activity', section: 'See natural wonders',
        requiredPlaces: [{ place: 'Þingvellir National Park', lat: 64.25, lng: -21.13, _keep: false, _origin: 'max' }]
      });
      window._tb.placeActivities.push({
        id: 'q-subplace', type: 'activity', section: 'More places to consider',
        requiredPlaces: [{ place: 'Reykjavik Old Harbour', lat: 64.1512, lng: -21.9402, _keep: false, _origin: 'max' }]
      });
      if (typeof window._reconcileUserListedKeeps === 'function') window._reconcileUserListedKeeps();

      function find(name) {
        let hit = null;
        (window._tb.placeActivities || []).forEach(function (it) {
          (it.requiredPlaces || []).forEach(function (p) {
            if (p && window._pmKey(p.place) === window._pmKey(name)) hit = p;
          });
        });
        return hit;
      }
      const info = window._resolveListedInfo('Þingvellir National Park');
      const oh = find('Reykjavik Old Harbour');
      return {
        resolvedRole: info && info.role,
        resolvedMatch: info && info.matchType,
        thingvellirKept: !!(find('Þingvellir National Park') || {})._keep,
        oldHarbourKept: !!(oh && oh._keep === true),
        oldHarbourOvernight: !!(oh && oh.overnight === true),
      };
    });
    expect(r.resolvedRole, 'the expanded sight resolves to the listed sight').toBe('see');
    // 'expanded' on first match; 'alias' once PD.401N has learned it (the
    // reconcile already ran the match, so the alias is recorded by now).
    expect(['expanded', 'alias'], 'matched by reverse-containment, possibly via the learned alias').toContain(r.resolvedMatch);
    expect(r.thingvellirKept, 'the user\'s listed sight is committed, not left unchecked').toBe(true);
    expect(r.oldHarbourKept, 'a stay\'s sub-place is NOT auto-checked (Max never checks)').toBe(false);
    expect(r.oldHarbourOvernight, 'a stay\'s sub-place is NOT promoted to an overnight').toBe(false);
  });

  // PD.401Q: kept listed sights with NO coordinates get geocoded so they
  // can pin (the "map is missing real-place pins" bug). Stubs fetch so the
  // test is deterministic and offline.
  test('PD.401Q: a kept sight with no coords is geocoded so it can pin', async ({ page }) => {
    await bootClean(page);
    await runPipeline(page);
    const r = await page.evaluate(async () => {
      // Deterministic geocoder: Goðafoss Waterfall → Goðafoss coords.
      const realFetch = window.fetch;
      window.fetch = async function (url) {
        if (String(url).indexOf('nominatim') !== -1) {
          return { json: async () => ([{ lat: '65.6826', lon: '-17.5500' }]) };
        }
        return realFetch.apply(window, arguments);
      };
      window._tb.placeActivities.push({
        id: 'q-nocoord', type: 'activity', section: "Sights you're keeping",
        requiredPlaces: [{ place: 'Goðafoss Waterfall', _keep: true, _origin: 'user' }]
      });
      // A render-triggered pass may have left the in-flight guard set (or a
      // tried flag) from a real fetch; reset so this explicit call runs.
      window._paGeocodeInFlight = false;
      const before = (function () {
        let p = null;
        window._tb.placeActivities.forEach(function (it) {
          (it.requiredPlaces || []).forEach(function (q) {
            if (q && q.place === 'Goðafoss Waterfall') p = q;
          });
        });
        return p && (typeof p.lat === 'number');
      })();
      await window.geocodeMissingPlaceActivityCoords();
      // Find the place by identity (it may have merged with a twin).
      let located = false;
      window._tb.placeActivities.forEach(function (it) {
        (it.requiredPlaces || []).forEach(function (q) {
          if (q && window._pmKey(q.place) === window._pmKey('Goðafoss Waterfall')) {
            if (typeof q.lat === 'number' && isFinite(q.lat)) located = true;
          }
        });
      });
      window.fetch = realFetch;
      return { before, located };
    });
    expect(r.before, 'the listed sight started with no coordinates').toBe(false);
    expect(r.located, 'after geocoding it has finite coordinates → can pin').toBe(true);
  });
