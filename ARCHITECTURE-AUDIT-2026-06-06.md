...
## 19. PD.401Q — every real place can be pinned; ONE identity for keep

Diagnosed from a dump of the live build's `placeActivities` (no more
guessing). Two reported symptoms — "Þingvellir is missing" and "the map
is missing real-place pins" — traced to ONE root.

**Root cause.** The geocoder (`geocodeMissingCoords`) only ever ran on
`_tb.candidates` — Max's suggestions. The sights built from the user's
*pasted list* live in `placeActivities` and never passed through any
geocoder, so they carried no `lat`/`lng`. A place with no coordinates
cannot be pinned — and those were precisely the kept (green) places, so
the map looked empty exactly where the user's committed sights belonged.
The same gap split each listed place from Max's coordinate-bearing
enriched twin ("Goðafoss Waterfall" vs "Goðafoss"): the identity merge is
coordinate-aware, and with no coords on the user's side it could not
confirm the match — so they stayed as two records (a duplicate), and the
twin that *did* have coords sat at `keep=false`.

**Þingvellir specifically** was a SECOND identity disagreeing with
coverage. Coverage matches bidirectionally (`PlaceKey.contains` both
ways), so it found "Þingvellir" inside "Þingvellir National Park" — the
audit reported 0 missing (reproduced). But keep-enforcement's
`_resolveListedInfo` only matched ONE direction (listed key *longer* than
the place) and its token-overlap matcher *skipped single-token listed
names*. So a one-word listed sight the LLM expanded was left at the LLM's
`keep=false` — the lost commit. Exactly the "two disagreeing identities"
the prior rounds kept surfacing.

**The fix (one root, three symptoms).**
1. `_resolveListedInfo` gains the missing direction: a one-word listed
   SIGHT matches its expanded form ("Þingvellir" → "Þingvellir National
   Park"), the same identity coverage uses — and learns the alias once
   (PD.401N). Gated to sights so a listed STAY can never reverse-match its
   own sub-place ("Reykjavik" ⇏ "Reykjavik Old Harbour"), which would
   auto-check a Max suggestion (a "Max never checks" violation).
2. `geocodeMissingPlaceActivityCoords` backfills coordinates for every
   kept `placeActivities` sight that lacks them (region-biased Nominatim,
   per-place tried flag, in-flight guard) — skipping routes, synthetic
   containers, and `_virtual` umbrella entries (Golden Circle and other
   regions/loops are not single pins; their component real places are).
   It runs from both place surfaces (picker map + trip overview).
3. After coords land, `_runPlaceSetPasses` re-canonicalizes — the now-
   located user sight merges with its Max coordinate twin by the existing
   coordinate-aware identity: one record, the user's keep wins, coords
   present → one green pin, no duplicate.

**Verified.** 115 contract checks (new Rules 32a–e), all Node suites, and
the Playwright build harness (two new PD.401Q tests: the expanded-twin is
committed while a stay's sub-place is not; a no-coord kept sight is
geocoded so it can pin) — 0 failures, "Max never checks" still holds.

**Residual (named, not hidden).** Twins whose names diverge by more than
a trailing qualifier and share only one distinctive token
("Jökulsárgljúfur Canyon" vs "Jökulsárgljúfur National Park") are not
auto-merged — both now pin (close together) once geocoded, but as two
records. Merging those safely needs a feature-type-word rule gated by
coordinates; deferred rather than shipped as a fragile heuristic.
